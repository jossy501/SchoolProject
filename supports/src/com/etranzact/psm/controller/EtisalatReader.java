...
public class EtisalatReader {
    ...
    public EtisalatReader(User user,String user_code, String clientIP, String user_id, int pinLength, String network, int totalCount, String fileName, java.io.File inputfullFileName, double totalPinAmount, List<TPsmStock> list, List<ErrorBean> invalid, String systemBatchNumber, String batchNumber) {
        ...
        List<ErrorBean> error = invalid;
           ...
           Connection con = Env.getConnection4PSMRPDB();
            ...
            while ((strLine = br.readLine()) != null) {
                ...
                String expdate="";
                t = t + 1;
                expdate = new DateUtility().getDate();
                ...
                ErrorBean errorBean = new ErrorBean();
                    ...
                    if(sn.trim().startsWith("5"))
                    {
                      amount = "50" ;
                    }
                    else if(sn.trim().startsWith("82") || sn.trim().startsWith("80"))
                    {
                      amount = "100" ;
                    }
                    else if(sn.trim().startsWith("2"))
                    {
                      amount = "200" ;
                    }
                    else if(sn.trim().startsWith("4"))
                    {
                      amount = "500" ;
                    }
                    else if(sn.trim().startsWith("1"))
                    {
                      amount = "1000" ;
                    }
...

                {
                   ...
                   stock.setPIN_DENO(amount.replace(".00", "").replace(".0", ""));
                    ...
                    stock.setPIN(pin);
                    stock.setPIN_EXPIRATION(new DateUtility().convertStringToDate(expdate));
...
//                                            + "'" + stock.getPinId() + "',getdate(), '" + user.getUser_id() + "', 'A','" + stock.getProviderId() + "','')";
    ...
    public static void main(String d[]) {
        try {
            FileInputStream fstream = new FileInputStream("C:\\Users\\akachukwu.ntukokwu\\Downloads\\sample%20pins\\sample pins\\etisalat sample.txt");
            DataInputStream in = new DataInputStream(fstream);
            BufferedReader br = new BufferedReader(new InputStreamReader(in));
            String strLine;
            while ((strLine = br.readLine()) != null) {
