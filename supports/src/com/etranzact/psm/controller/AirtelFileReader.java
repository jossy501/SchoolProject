...
class AirtelFileReader {
    ...
    public AirtelFileReader(User user,String user_code, String clientIP, String user_id, int pinLength, String network, int totalCount, String fileName, java.io.File inputfullFileName, double totalPinAmount, List<TPsmStock> list, List<ErrorBean> invalid, String systemBatchNumber, String batchNumber) {
        ...
        List<ErrorBean> error = invalid;
            ...
            while ((strLine = br.readLine()) != null) {
                ...
                ErrorBean errorBean = new ErrorBean();
                        errorBean.setErrorDesc("No Colunm for Pin Serial Number "+sn);
                        errorBean.setLine(t + "");
                        error.add(errorBean);
                }
                try {
                    ...
                    ErrorBean errorBean = new ErrorBean();
                        errorBean.setErrorDesc("No Colunm for Pin Number "+pin);
                        errorBean.setLine(t + "");
                        error.add(errorBean);
                }
                try {
                    amount = rec[2].trim();
                    double t_=Double.parseDouble(amount)/100;
                    amount=t_+"";
                } catch (Exception sd) {
                    sd.printStackTrace();
                    ErrorBean errorBean = new ErrorBean();
                        errorBean.setErrorDesc("No Colunm for Pin Amount or Invalid pin amount "+amount);
                        errorBean.setLine(t + "");
                        error.add(errorBean);
                }
...

                {
                    ...
                    //System.out.println(stock.getPIN_DENO()+" XXXXXXXXXXXXXXXXX XXXXXX"+amount+" XXXXXXXX XXXXXXXXXXXXXXXX "+stock.getPIN_VALUE());
                    stock.setPIN(pin);
...
//                                            + "'" + stock.getPinId() + "',getdate(), '" + user.getUser_id() + "', 'A','" + stock.getProviderId() + "','')";
                                ...
                                try{
                                System.out.println(stock.getPIN().length()+" Invalid Pin Length Invalid Pin Length XXXXXXXXXXXXXXX");
                                System.out.println(stock.getPIN().substring(0,10));
                                }catch(Exception sd){}
                                errorBean.setLine(t + "");
                                error.add(errorBean);
    ...
    public static void main(String d[]) {
        try {
//            FileInputStream fstream = new FileInputStream("C:\\Users\\akachukwu.ntukokwu\\Downloads\\sample%20pins\\sample pins\\zain sample.txt");
//            DataInputStream in = new DataInputStream(fstream);
//            BufferedReader br = new BufferedReader(new InputStreamReader(in));
//            String strLine;
//            while ((strLine = br.readLine()) != null) {
...
//                try {
//                    amount = rec[2].trim();
//                    double t=Double.parseDouble(amount)/100;
//                    amount=t+"";
//                } catch (Exception sd) {
//                }
//
...
//            in.close();
            ...
            System.out.println("5021920848960082188".substring(6,12));
        } catch (Exception sd) {
        }
    }
}
