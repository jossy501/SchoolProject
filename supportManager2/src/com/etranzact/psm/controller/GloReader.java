...
public class GloReader {
    ...
    public GloReader(User user,String user_code, String clientIP, String user_id, int pinLength, String network, int totalCount, String fileName, java.io.File inputfullFileName, double totalPinAmount, List<TPsmStock> list, List<ErrorBean> invalid, String systemBatchNumber, String batchNumber) {
    ...
    public List<String> readFile(User user,String user_code, String clientIP, String user_id, int pinLength, String network,
        ...
        List<ErrorBean> error = invalid;
            ...
            while ((strLine = br.readLine()) != null) {
                ...
                String pin = "";
                String amount = "";
                String xpdate = new DateUtility().getDate();
                t=t+1;
                if(!strLine.trim().equals("|"))
                {
                try {
                    sn = rec[4].trim();
                    sn=sn.toUpperCase().replace("SENUM=", "");
                ...
                try {
                    pin = rec[2].trim()+rec[3].trim();
                    pin=pin.toUpperCase().replace("PIN=","").replace("SCNUM=", "").replace("\"", "");
                ...
                try {
                    ...
                    amount = rec[5].trim();
                    amount=amount.toUpperCase().replace("SCRPREF=", "");

                } catch (Exception sd) {
                    ErrorBean errorBean = new ErrorBean();
                        errorBean.setErrorDesc("No Colunm for Pin amount");
                        errorBean.setLine(t + "");
                        error.add(errorBean);
                }
                try {
                    ...
                    xpdate = rec[1].trim();
                    xpdate=xpdate.toUpperCase().replace("END_DATE=", "");
                    String[] sd = xpdate.split("/");
                    xpdate=sd[2]+"/"+sd[1]+"/"+sd[0];
...

                {
                   ...
                   stock.setPIN_DENO(amount.replace(".00", "").replace(".0", ""));
                    ...
                    stock.setPIN(pin);
                    stock.setPIN_EXPIRATION(new DateUtility().convertStringToDate(xpdate));
...
//                                            + "'" + stock.getPinId() + "',getdate(), '" + user.getUser_id() + "', 'A','" + stock.getProviderId() + "','')";
    ...
    public static void main(String d[]) {
        try {
            FileInputStream fstream = new FileInputStream("C:\\Users\\akachukwu.ntukokwu\\Downloads\\sample%20pins\\sample pins\\glo sample.txt");
            DataInputStream in = new DataInputStream(fstream);
            BufferedReader br = new BufferedReader(new InputStreamReader(in));
            String strLine;
            while ((strLine = br.readLine()) != null) {
                ...
                try {
                    pin = rec[2].trim()+rec[3].trim();
                    pin=pin.toUpperCase().replace("PIN=","").replace("SCNUM=", "");
                } catch (Exception sd) {
                }
